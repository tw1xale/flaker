...
pub fn nixos_rebuild_switch(flake_target: &str, dir: &Path) -> Result<()> {
    let needs_sudo = crate::actions::libc_getuid() != 0;
    let mut cmd = make_cmd("nixos-rebuild", dir, needs_sudo);
    cmd.args(["switch", "--flake", flake_target]);

    let desc = if needs_sudo {
        format!("sudo nixos-rebuild switch --flake {flake_target}")
    } else {
        format!("nixos-rebuild switch --flake {flake_target}")
    };

    let status = run_visible("REBUILDING & ACTIVATING SYSTEM", &desc, &mut cmd)
    ...
    for sub in &["bin", "sbin"] {
        ...
        for entry in entries.flatten() {
            ...
            if target_before != target_after {
                ...
                let Some((_hash, pkg_name_ver)) = store_dir.split_once('-') else {
                    continue;
                };
...
pub fn cleanup_nix_store() -> Result<()> {
    ...
    let mut cmd1 = make_cmd("nix-collect-garbage", Path::new("."), needs_sudo);
    cmd1.arg("-d");
    let s1 = run_visible(
    ...
    let mut cmd2 = make_cmd("nix-store", Path::new("."), needs_sudo);
    cmd2.arg("--gc");
    let s2 = run_visible(
    ...
    let mut cmd3 = make_cmd("nix-store", Path::new("."), needs_sudo);
    cmd3.arg("--optimise");
    let s3 = run_visible(
